...
{
    path_abs = ros::package::getPath("propbot_mission") + path_local;
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/propbot_mission/waypoint_following_status", 100);
    ...
    std::string path_local = "/missions/test_mission_1.txt";
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        map_point.header.frame_id = "odom";
        map_point.header.stamp =  ros::Time(0);
        map_point.point.x = latiGoal;
        map_point.point.y = longiGoal;
   
        map_next.header.frame_id = "odom";
        map_next.header.stamp =  ros::Time(0);
        map_next.point.x = latiNext;
        map_next.point.y = longiNext;
        ...
        {
            ROS_INFO("Propbot has reached its goal!");
            //switch to next waypoint and repeat
        }
        else
        {
            ROS_ERROR("Propbot was unable to reach its goal. Waypoint unreachable.");
    ...
    ROS_INFO("Propbot has reached all of its goals!!!\n");
