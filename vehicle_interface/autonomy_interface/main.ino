...
std_msgs::UInt8 dir;    //0 means backward, 1 means forward

//Defining Publisher and callback
ros::Publisher talker_left("lw", &speed_Left);
ros::Publisher talker_right("rw", &speed_Right);
ros::Publisher talker_dir("dr", &dir);

void callback_left ( const std_msgs::UInt8& msg_left){
speed_Left.data = msg_left.data;
talker_left.publish( &speed_Left );
}

void callback_right ( const std_msgs::UInt8& msg_right){
speed_Right.data = msg_right.data;
talker_right.publish( &speed_Right );
}

void callback_dir ( const std_msgs::UInt8& msg_dir){
dir.data = msg_dir.data;
talker_dir.publish( &dir );
}
...
ros::Subscriber<std_msgs::UInt8> listener_dir("listener_direction", callback_dir);
...
void setup()
{
    ...
    node.advertise(talker_dir);
    node.subscribe(listener_dir);
...
void loop()
{
    ...
    node.spinOnce();
        delay(2);
    if (distance > MIN_DISTANCE){//safe distance
    // Update wheel commands
      if(sw_b < RC_SWX_HIGH_MAX && sw_b > RC_SWX_HIGH_MIN){//autonomy mode
        //node.spinOnce();
        //delay(2);
        if(dir.data == 0){
          left_dir = DIR_BW;
          right_dir = DIR_BW;
          left_duty = 1 - linMapToFloat(speed_Left.data, Min_Speed, Max_Speed, 0, 1);
          right_duty = 1 - linMapToFloat(speed_Right.data, Min_Speed, Max_Speed, 0, 1);
          }
        else{
          left_dir = DIR_FW;
          right_dir = DIR_FW;
          left_duty = linMapToFloat(speed_Left.data, Min_Speed, Max_Speed, 0, 1);
          right_duty = linMapToFloat(speed_Right.data, Min_Speed, Max_Speed, 0, 1);
          }
        commands = {{
    ...
    else{ //unsafe distance, the car should shut down
